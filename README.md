...
이 저장소의 샘플 게임은 콘텐츠 자체보다 **Tick, State Sync, AOI, Pooling, Diagnostics** 구조를 검증하기 위한 테스트베드 역할을 합니다.
...
- AOI / Pooling / Diagnostics HUD 지원
...
이 구조는 다음 문제를 줄이기 위해 설계했습니다.

- 게임 코드가 네트워크 구현 세부사항에 직접 묶이는 문제
- 메시지 처리와 전송 처리의 책임이 섞이는 문제
- 특정 Transport 구현 변경 시 상위 계층 수정 범위가 커지는 문제
- 지속 상태와 일회성 이벤트가 뒤섞여 디버깅이 어려워지는 문제
- 성능 측정과 문제 재현 경로가 불분명해지는 문제
...
- 메시지 정책 분리
- AOI / Pooling / Diagnostics 기반 검증 가능성
